...
class OptimizeProject:
    ...
    def optimize_command_groups(self, method: OptimizationMethod) -> List[Command[PlotterPosition]]:   
        ...
        initial_solution = [Point(point.command_detail.posX, point.command_detail.posY, point.command_detail.hitCount) for point in self.all_commands]
    ...
    def _optimize_command_groups(self, labels: ndarray, unique_labels: List[int], command_groups: List[CommandGroup], method: OptimizationMethod) -> List[Point]:
        ...
        for command_group in solution:
            optimized_commands = optimized_commands + [Command(PlotterPosition(command.command_detail.posX, command.command_detail.posY, command.command_detail.hitCount)) for command in command_group.commands]
        
        non_optimized_commands: List[Command] = []
        for command_group in init_solution:
            non_optimized_commands = non_optimized_commands + [Command(PlotterPosition(command.command_detail.posX, command.command_detail.posY, command.command_detail.hitCount)) for command in command_group.commands]
